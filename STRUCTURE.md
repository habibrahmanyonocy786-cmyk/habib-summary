# Habib-summary - Project Structure

> Last updated: 12 June 2026 (2026-06-12 09:28)

```
habib-summary/
+-- data.json (18.8 KB)
+-- DB-backend.md (45.5 KB)
+-- index.html (3.6 KB)
+-- PLAN.md (10.8 KB)
+-- README.md (0.8 KB)
+-- STRUCTURE.md (2.1 KB)
+-- SUMMARY.md (2.1 KB)
+-- update-project-files.ps1 (6.3 KB)
+-- update-structure.ps1 (1 KB)
+-- _config.yml (0.2 KB)
    +-- login.html (2.6 KB)
    +-- manage.html (2.1 KB)
        +-- custom.css (2.5 KB)
        +-- .gitkeep (0 KB)
        +-- app.js (45.7 KB)
        +-- extract-text.js (1.3 KB)
        +-- ocr.js (0.8 KB)
        +-- summarizer.js (2.1 KB)
        +-- viewer-docx.js (0.5 KB)
        +-- viewer-pdf.js (1.2 KB)
    +-- .gitkeep (0 KB)
    +-- correction-plan.md (8.7 KB)
        +-- footer.html (0.2 KB)
        +-- header.html (0.6 KB)
        +-- nav-tabs.html (1 KB)
        +-- sanitize-html.js (1.1 KB)
        +-- storage.js (1 KB)
```

## File Details

| File | Size (KB) | Description |
|------|-----------|-------------|
| data.json | 18.8 | - |
| DB-backend.md | 45.5 | - |
| index.html | 3.6 | Main SPA app with 10 books & 30 articles |
| PLAN.md | 10.8 | Database & backend plan |
| README.md | 0.8 | Project README |
| STRUCTURE.md | 2.1 | File structure doc |
| SUMMARY.md | 2.1 | Project summary |
| update-project-files.ps1 | 6.3 | Combined structure + summary updater |
| update-structure.ps1 | 1 | Legacy structure script |
| _config.yml | 0.2 | GitHub Pages config |
| admin\login.html | 2.6 | - |
| admin\manage.html | 2.1 | - |
| assets\css\custom.css | 2.5 | - |
| assets\fonts\.gitkeep | 0 | - |
| assets\js\app.js | 45.7 | - |
| assets\js\extract-text.js | 1.3 | - |
| assets\js\ocr.js | 0.8 | - |
| assets\js\summarizer.js | 2.1 | - |
| assets\js\viewer-docx.js | 0.5 | - |
| assets\js\viewer-pdf.js | 1.2 | - |
| docs\.gitkeep | 0 | - |
| docs\correction-plan.md | 8.7 | Code correction plan (16 issues) |
| src\components\footer.html | 0.2 | - |
| src\components\header.html | 0.6 | - |
| src\components\nav-tabs.html | 1 | - |
| src\utils\sanitize-html.js | 1.1 | - |
| src\utils\storage.js | 1 | - |

---

**Developer:** Habib Rahman Yonocy
**Contact:** 0784258414
